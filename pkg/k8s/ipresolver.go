...
func (resolver *K8sIPResolver) handlePodWatchEvent(podEvent *watch.Event) {
	clearPodMetrics := func(namespace, podName string) {
		// Delete all metrics with the same namespace and pod_name
		podMetrics.DeletePartialMatch(prometheus.Labels{
			"namespace": namespace,
			"pod_name":  podName,
		})
	}
	...
	case watch.Deleted:
		if val, ok := podEvent.Object.(*v1.Pod); ok {
			...
			clearPodMetrics(val.Namespace, val.Name)
		}
	}
}
